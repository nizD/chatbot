...
#setting up the LOssErros,the optimizer and the lost gradient cliiping
...
#splitting the data in to batch of questions and answers
def split_into_batches(questions,answers,batch_size):
    for batch_index in range(0,len(questions)//batch_size):
        start_index = batch_index * batch_size
        questions_in_batch = questions[start_index,start_index + batch_size]
        answers_in_batch =answers[start_index,start_index + batch_size]
        padded_question_in_batch  = apply_padding(questions_in_batch,questionswords2int)
        padded_answer_in_batch = apply_padding(answers_in_batch,answerswords2int)
        
        yield padded_question_in_batch,padded_answer_in_batch
        
    
#splitting the question and answers into training and valiidation sets
training_validation_split = int(len(sorted_clean_questions))
...
checkpoints = 'chatbot_weights.ckpt'
    ...
    for batch_index,(padded_questions_in_batch,padded_answers_in_batch) in enumerate(split_into_batches(training_questions,training_answers,batch_size)):
        ...
        if batch_index % batch_index_check_validation_loss == 0 and batch_index > 0:
            ...
            for batch_index_validation,(padded_questions_in_batch,padded_answers_in_batch) in enumerate(split_into_batches(validation_questions,validation_answers,batch_size)):
                batch_loss_error = session.run(loss_error,{inputs:padded_questions_in_batch,
                                                           targets:padded_answers_in_batch,
                                                           lr :learning_rate,
                                                           sequence_length:padded_answers_in_batch.shape[1],
                                                           keep_prob: 1})
            ...
            print('Validations Loss Error : {>6.3f} , Batch Validation Time : {%d} seconds'.format(avg_validation_loss_error,
                  int(batch_time)))
            
            #applying some delay and early_stopping
            ...
            if avg_validation_loss_error <=min(list_validation_loss_error):
                ...
                saver.save(session,checkpoints)
            else:
                print("Sorry i could not speak better,I need to practice")
                early_stopping_check +=1
                if early_stopping_check == early_stopping_stop:
                    break
    if early_stopping_check == early_stopping_stop:
        print('My apologies,I cannot speak anymore.This is the best I can do')
        break
print('EndGame')
